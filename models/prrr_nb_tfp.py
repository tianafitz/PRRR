...
def rrr(X, q, k):
    ...
    A = yield tfd.Gamma(concentration=tf.fill([p, k], 2.0),
                 rate=tf.ones([p, k]),
                 name="A")

    B = yield tfd.Gamma(concentration=tf.fill([k, q], 2.0),
                 rate=tf.ones([k, q]),
                 name="B")

    Y = yield tfd.Poisson(rate=tf.matmul(tf.matmul(X.astype("float32"), A), B),
                     name="Y")
...
def fit_rrr(X, Y, k):
    ...
    def target_log_prob_fn(A, B): return model.log_prob((A, B, Y))
...
def naive_bayes(X, q):
    ...
    lam = yield tfd.Gamma(concentration=tf.fill([p, q], 2.0),
                 rate=tf.ones([p, q]),
                 name="A")

    Y = yield tfd.Poisson(rate=tf.matmul(X.astype("float32"), lam),
                        name="Y")
...
def fit_naive_bayes(X, Y):
    ...
    def target_log_prob_fn(lam): return model.log_prob(
        (lam, Y))
    ...
    def factored_normal_variational_model():
      qlam = yield tfd.Gamma(concentration=qlam_concentration,
                 rate=qlam_rate,
                 name="qlam")
