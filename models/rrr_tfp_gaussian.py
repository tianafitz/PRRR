...
def rrr(X, q, k):
    ...
    A = yield tfd.Normal(loc=tf.zeros([p, k]),
                 scale=tf.ones([p, k]),
                 name="A")

    B = yield tfd.Normal(loc=tf.zeros([k, q]),
                 scale=tf.ones([k, q]),
                 name="B")

    Y = yield tfd.Normal(loc=tf.matmul(tf.matmul(X.astype("float32"), A), B),
                        scale=tf.ones([n, q]),
                     name="Y")
...
def fit_rrr(X, Y, k):
    ...
    def target_log_prob_fn(A, B): return model.log_prob(
        (A, B, Y))
...
def naive_bayes(X, q):
    ...
    lam = yield tfd.Normal(loc=tf.zeros([p, q]),
                 scale=tf.ones([p, q]),
                 name="A")

    Y = yield tfd.Normal(loc=tf.matmul(X.astype("float32"), lam),
                        scale=tf.ones([n, q]),
                        name="Y")
