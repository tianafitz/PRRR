...
def computeCV(y1, y2):
    return scipy.stats.variation(
        np.reshape(y1 - y2, (np.shape(y1)[0] * np.shape(y1)[1]))
    )
...
def update_all_zqk(
    ...
    for k1 in range(k_limit):
        ...
        if all(abs(phi_mean[k1, :]) < threshold):
            ...
            resids = np.dot(
                (knowny - np.dot(np.dot(knownx, new_nus[:, not_k]), am[not_k, :])),
                am[k1, :],
            )
            for q1 in range(q):
                mask = np.ones(all_q.shape, dtype=bool)
...
def update_all_ak_va(
    ...
    for q1 in range(q):
        ...
        for k1 in range(k_limit):
            sumvs_k[k1] = sumvs_k[k1] + np.sum(
                np.dot(
                    zm[q1, k1] * knownx[:, q1], np.dot(knownx[:, notqi], zm[notqi, k1])
                )
            )
    ...
    for k1 in range(k_limit):
        ...
        if all(nu[:, k1] < threshold):
            ...
            sump = np.sum(
                np.multiply(
                    np.tile(np.dot(knownx, zm[:, k1]), (p, 1)).T,
                    (
                        knowny
                        - np.dot(np.dot(knownx, zm[:, not_k]), phi_means[not_k, :])
                    ),
                ),
                0,
            )
            phi_vars[k1, :] = np.power(
                ((kappa1 / kappa2[k1, :]) + (sumvs_k[k1] / vn)), -1
            )
...
def geweke(trace, alpha):
    ...
    if ntrace < 10:
        ...
        try:
            z = (np.mean(trace[seta]) - np.mean(trace[setb])) / np.sqrt(
                (np.std(trace[seta]) / sum(seta)) + (np.std(trace[setb]) / sum(setb))
            )
...
def fitModel(
    ...
    exp_phiphis = np.zeros((int(k_limit)))
    # Expectation of Ak. Ak.'
    dgtaus = np.zeros((int(k_limit)))
    # Digamma functions and subtraction of Taus
    xxt_qs = np.zeros((int(q)))
    # X.q'X.q
    uz_sum2s = np.zeros((int(q)))
    trace_varnus = 0
    # Trace of the covariance of Nu (sum of the variances since its diagonal)
    ...
    for i in range(iterations):
        ...
        # Update A
        if learn_a:
            phi_mean, phi_var = update_all_ak_va(
                kappa1,
                kappa2,
                var_n,
                x,
            ...
            for k1 in range(k_limit):
                exp_phiphis[k1] = np.sum((phi_var[k1, :])) + np.dot(
                    phi_mean[k1, :], phi_mean[k1, :].T
                )
        ...
        for k1 in range(k_limit):
            ...
            dgtaus[k1] = scipy.special.digamma(tau_k1[k1]) - scipy.special.digamma(
                tau_k2[k1]
            )
            trace_varnus = trace_varnus + (tau_k1[k1] * tau_k2[k1]) / (
                math.pow(tau_k1[k1] + tau_k2[k1], 2) * (tau_k1[k1] + tau_k2[k1] + 1)
            )
        ...
        if i % 100 == 0 and i > burn:
            ...
            if (
                ...
                and phim_con
                and phiv_con
                and ((avar_con and learn_avar) or not learn_avar)
            ):
...
# p = 20
# q = 30
# x = np.random.choice([0, 1, 2], size=(n, q), replace=True)
...
# rank = np.sum(
#     (
#         (np.sum(sampled_z > threshold, axis=0) > 0)
#         * (np.sum(phi_mean_traces[:, :, numiter - 1] > threshold, axis=1) > 0)
#     )
# )
...
#                 x,
#                 np.dot(
#                     nu_traces[:, :, numiter - 1], phi_mean_traces[:, :, numiter - 1]
#                 ),
#             )
#         ),
#         interpolation="none",
...
#     plt.clf()
#     plt.plot(np.tile(list(range(numiter)), (k_limit, 1)).T, tau_traces1.T)
#     plt.plot(np.tile(list(range(numiter)), (k_limit, 1)).T, tau_traces2.T)
#     plt.ylabel("Tau k")
...
#         plt.plot(list(range(numiter)), phi_mean_traces[ki, :, :].T)
#     plt.ylabel("Phi kp")
#     plt.xlabel("Iteration")
...
#     plt.clf()
#     for ki in range(k_limit):
#         plt.plot(np.tile(list(range(numiter)), (q, 1)).T, nu_traces[:, ki, :].T)
#     plt.ylabel("Nu qk")
#     plt.xlabel("Iteration")
