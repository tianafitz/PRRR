...
keep_factor = sampled_z.sum(0) > 0
nu_traces_filtered = nu_traces[:, :, numiter - 1][:, keep_factor]
...
Z_est = nu_traces_filtered.T

latent_vars = x @ Z_est.T


# cell_types = adata.var.index.values[gene_idx]
rank = latent_vars.shape[1]
n_cell_types = len(cell_types)



Z_df = pd.DataFrame(Z_est.T)
...
# plt.tight_layout()
plt.savefig("./out/pairplot_berrri.png")
plt.show()
